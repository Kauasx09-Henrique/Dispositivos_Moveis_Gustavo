...
export default function ListaReceitas({ navigation, route }) {
  const verDetalhes = (receita) => {
    console.log("Ver detalhes da receita:", receita.nome);
  };
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Card style={styles.card}>
            <Card.Title
              title={item.nome}
              subtitle={`Tempo de preparo: ${item.tempoPreparo}`}
              left={(props) => (
                <Avatar.Image {...props} source={{ uri: item.imagem }} />
              )}
            />
            <Card.Content>
              <Text style={styles.porcoes}>Porções: {item.porcoes}</Text>
              <Button
                mode="contained"
                onPress={() =>
